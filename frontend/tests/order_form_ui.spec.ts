// Generated by Skyramp v1.3.16 on 2026-03-30 04:07:33.77887503 +0000 UTC m=+288.654969272
// Command: skyramp generate ui rest \
// 		--framework playwright \
// 		--language typescript \
// 		--output order_form_ui.spec.ts \
// 		--output-dir /home/runner/work/demoshop-testbot/demoshop-testbot/frontend/tests \
// 		--overwrite \
// 		--playwright-trace /home/runner/work/demoshop-testbot/demoshop-testbot/frontend/public/traces/ui_test_playwright.zip

// Import of required libraries
import { test } from '@playwright/test';
import { expect, getValue, newSkyrampPlaywrightPage } from '@skyramp/skyramp';

const pageTimeout = 15000;

async function createProduct(page, name, description, image_url, category, price) {
    await page.getByRole("button", { name: "Add Product" }).click();
    await page.getByTestId("new_product_name").click();

    const playwrightRequest0: Record<string, any> = {
        "category": category,
        "description": description,
        "image_url": image_url,
        "name": name,
        "price": price
    };

    await page.getByTestId("new_product_name").fill(getValue(playwrightRequest0, "name"));
    await page.getByTestId("new_product_name").press("Tab");
    await page.getByTestId("new_product_description").fill(getValue(playwrightRequest0, "description"));
    await page.getByTestId("new_product_description").press("Tab");
    await page.getByTestId("new_product_image_url").fill(getValue(playwrightRequest0, "image_url"));
    await page.getByTestId("new_product_image_url").press("Tab");
    await page.getByTestId("new_product_category").fill(getValue(playwrightRequest0, "category"));
    await page.getByTestId("new_product_category").press("Tab");
    await page.getByRole("combobox").press("Tab");
    await page.getByTestId("new_product_price").fill(getValue(playwrightRequest0, "price").toString());
    // Wait to finish any potential hydration
    await page.waitForTimeout(1500);
    const responsePromise0 = page.waitForResponse("**/api/v1/products**");
    await page.getByRole("button", { name: "Submit" }).click();
    const response0 = await responsePromise0;

    // Wait for network activities to finish
    await page.waitForTimeout(3000);
}

async function createOrderWithSkyPhone1(page, customer_email) {
    await page.getByTestId("add-order-btn").click();
    await page.getByTestId("add-order-input-email").click();

    const playwrightRequest1: Record<string, any> = {"customer_email": customer_email};

    await page.getByTestId("add-order-input-email").fill(getValue(playwrightRequest1, "customer_email"));
    await page.getByTestId("add-order-label-products").click();
    await page.getByRole("combobox").click();
    await page.getByTestId("add-order-product-option-SkyPhone 1").click();
    await page.getByTestId("add-order-products-container").getByRole("button", { name: "Add" }).click();
    // Wait to finish any potential hydration
    await page.waitForTimeout(1500);
    const responsePromise1 = page.waitForResponse("**/api/v1/orders**");
    await page.getByTestId("add-order-form-fields-container").getByRole("button", { name: "Add Order" }).click();
    const response1 = await responsePromise1;
}

test('testUi', async ({ page }) => {
    test.setTimeout(192000);

    // set default timeout for all locator/page actions in this test
    page.setDefaultTimeout(pageTimeout);
    // Wrap playwright page with Skyramp page
    page = newSkyrampPlaywrightPage(page);

    await page.goto("https://demoshop.skyramp.dev/products");
    await createProduct(page, "SkyPhone 1", "The ultimate smartphone - powered by Skyramp", "https://images.google.com/skyphone1", "Phones", 999);

    await expect(page.getByTestId("product-detail-value-price")).toContainText("$999");
    await page.getByTestId("navbar-orders").click();
    await createOrderWithSkyPhone1(page, "sahil@skyramp.dev");

    const responsePromise2 = page.waitForResponse("**/api/v1/orders/**");
    await page.getByRole("button", { name: "Cancel Order" }).click();
    const response2 = await responsePromise2;

    await page.getByTestId("navbar-products").click();
    await page.getByTestId("product-SkyPhone-1-view-details").click();
    // Wait to finish any potential hydration
    await page.waitForTimeout(1500);
    const responsePromise3 = page.waitForResponse("**/api/v1/products/**");
    await page.getByRole("button", { name: "Delete Product" }).click();
    const response3 = await responsePromise3;
});
